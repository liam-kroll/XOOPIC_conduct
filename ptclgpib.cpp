...
void ParticleGroupIBES::advance(Fields& fields, Scalar dt)
{
  ...
  for (int i=0; i<n; i++, x++, u++){
    ...
    *u += Vector3(a,0,0); // full acceleration
    //CHANGED BY LIAM (added collision model and fields to arguments)
    //dxMKS = grid.differentialMove(fields.getMKS(*x), *u, *u, dt);
    dxMKS = fields.differentialMove(fields.getMKS(*x), *u, *u, dt, ParticleGroup::species->get_collisionModel(), this, i);
    //END CHANGE
...
void ParticleGroupIBEM::advance(Fields& fields, Scalar dt){
  ...
  for (int i=0; i<n; i++, x++, u++){
    ...
    *u += Vector3(a,0,0); // full acceleration
    ...
    dxMKS = fields.differentialMove(fields.getMKS(*x), *u*igamma, *u, dt, ParticleGroup::species->get_collisionModel(), this, i);
    //END CHANGE
