...
CollisionModel SpeciesParams::get_collisionModel()
{
	...
	if ((collisionModel.getValue()==(ostring)"electron")|(collisionModel.getValue()==(ostring)"1"))
		...
		model = boltzmann;
	//CHANGED BY LIAM (added all lines)
	else if ((collisionModel.getValue()==(ostring)"conduct_elec")|(collisionModel.getValue()==(ostring)"5"))
		model = conduct_elec;
	else if ((collisionModel.getValue()==(ostring)"conduct_ion")|(collisionModel.getValue()==(ostring)"6"))
		model = conduct_ion;
	//END CHANGE
