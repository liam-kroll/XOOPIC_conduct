...
Boundary* Emitter::initialPush(Scalar del_t, Scalar dt, Particle &p)
{
	...
	Vector3 dxMKS = fields->differentialMove_std(fields->getMKS(x), u0*igamma, u0, del_t);
	//Vector3 dxMKS = fields->differentialMove(fields->getMKS(x), u0*igamma, u0, del_t, Emitter::species->get_collisionModel());
	//END CHANGE
