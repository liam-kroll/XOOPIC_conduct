...
#include <grid.h>
#include <boltzman.h>
//CHANGED BY LIAM (included ParticleGroup class)
#include <ptclgrp.h>
//END CHANGE
#include "ngd.h"
...
//CHANGED BY LIAM (ADDED THIS STUFF FROM grid.h)
...
public:
	...
	//CHANGED BY LIAM (added collision model to arguments, changed v to non constants)
	//Vector3 differentialMove(const Vector2& x0, const Vector3& v, Vector3& u,
	//		Scalar dt) {return grid->differentialMove(x0, v, u, dt);}
	Vector3 differentialMove_std(const Vector2& x0, const Vector3& v, Vector3& u,
			Scalar dt) {return grid->differentialMove(x0, v, u, dt);}
	inline Vector3	differentialMove(const Vector2& x0,const Vector3& v, Vector3& u,
			Scalar dt, int collType,ParticleGroup *pg,int part_ind);
	//END CHANGE
...
//CHANGED BY LIAM (MOVED differentialMove member func to fields.h,
//		   then added a bunch of stuff. see comparison with
//		   commented version in grid.h.). A note is that this
//		   feature assumes only one conduct_ion species is present
...
inline Vector3 Fields::differentialMove(const Vector2& x0,const Vector3& v, Vector3& u,
		Scalar dt, int collType,ParticleGroup* pg, int part_ind)
{
	int geometryFlag=grid->query_geometry();
	Vector3 xf_cart(x0.e1()+v.e1()*dt, x0.e2()+v.e2()*dt, v.e3()*dt);
	switch(geometryFlag)
	{
	case ZXGEOM:
		if (collType==5) //particle being moved has collision type 'conduct_elec'
		{
			Vector2 test_r=grid->getNearestGridCoords(Vector2(xf_cart.e1(),xf_cart.e2()));
			Vector2 test_e1=grid->getNearestGridCoords(Vector2(xf_cart.e1(),x0.e2()));
			Vector2 test_e2=grid->getNearestGridCoords(Vector2(x0.e1(),xf_cart.e2()));
  			// find the species where the collisionModel is a conduct_ion and find the rho is adjacent cells
  			oopicListIter<Species> siter(*speciesList);
  			for (siter.restart(); !siter.Done(); siter++)
			{
    				Species* species = siter.current();
    				int i = species->get_collisionModel();
    				if (i==6)
				{
					if (get_rhospecies(species->getID(),test_r.e1(),test_r.e2())!=0.0)
					//		|| get_rhospecies(species->getID(),x0.e1(),x0.e2())==0.0)
					{
						//if conduct_ions exist where differentialMove vector places conduct_elec
						//at next timestep, then allow conduct_elec to move as normal.
						//the conduct_elec will be allowed to move as normal if it finds itself (somehow)
						//in a cell with no ions.
						break;
					}
					else if (get_rhospecies(species->getID(),test_e1.e1(),test_e1.e2())!=0.0)
					{
						//if conduct_ions exist where projection of differentialMove vector
						//onto the e1 coord places conduct_elec at next timestep, then allow 
						//conduct_elec to move along this projection, and reflect velocity
						//in e2 direction.
						xf_cart.set_e2(x0.e2());
						pg->set_v(Vector3(v.e1(),-1.0*v.e2(),v.e3()),part_ind);
						break;
					}
					else if (get_rhospecies(species->getID(),test_e2.e1(),test_e2.e2())!=0.0)
					{
						//if conduct_ions exist where projection of differentialMove vector
						//onto the e2 coord places conduct_elec at next timestep, then allow 
						//conduct_elec to move along this projection, and reflect velocity
						//in e1 direction.
						xf_cart.set_e1(x0.e1());
						pg->set_v(Vector3(-1.0*v.e1(),v.e2(),v.e3()),part_ind);
						break;
					}
					else
					{
						//if conduct_ions do not exist where projection of differentialMove vector
						//onto the e1 or e2 coord places conduct_elec at next timestep, then don't move 
						//conduct_elec, and reflect velocity totally.
						xf_cart.set_e1(x0.e1());
						xf_cart.set_e2(x0.e2());
						pg->set_v(Vector3(-1.0*v.e1(),-1.0*v.e2(),v.e3()),part_ind);
						break;
					}
				}
			}
		}
		return	Vector3(xf_cart.e1()-x0.e1(), xf_cart.e2()-x0.e2(), xf_cart.e3());
	...
	default:
	{
		...
		if (collType==5)
		{
			Vector2 test_r=grid->getNearestGridCoords(Vector2(xf.e1(),xf.e2()));
			Vector2 test_e1=grid->getNearestGridCoords(Vector2(xf.e1(),x0.e2()));
			Vector2 test_e2=grid->getNearestGridCoords(Vector2(x0.e1(),xf.e2()));
  			// find the species where the collisionModel is a conduct_ion and find the rho is adjacent cells
  			oopicListIter<Species> siter(*speciesList);
  			for (siter.restart(); !siter.Done(); siter++)
			{
